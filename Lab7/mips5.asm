...
loopInit:	
	li $s0,	-100000			# s0 stores the greatest value
...
loop:	
	beq $sp, 0x7fffeffc, endLoop	# while stack isn't empty
	lw $t0, 00($sp)			# get the top of the stack
	blt $s0, $t0, update1
afterUpdate1:	
	bgt $s2, $t0, update2
afterUpdate2:	
	addi $sp, $sp, +4		# pop the top of the stack
	addi $s4, $s4, -1		# update the top's index
	j loop
update1:		
	add $s0, $zero, $t0		# update greatest
	add $s1, $zero, $s4
	j afterUpdate1
update2:		
	add $s2, $zero, $t0		# update smallest
	add $s3, $zero, $s4
	j afterUpdate2
endLoop:	
printGreatest:	
